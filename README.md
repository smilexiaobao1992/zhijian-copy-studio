...
- 微信公众号富文本样式工坊：8 套版式气质、7 种标题样式，可再微调字体、字号、行距、主题色和代码明暗
- 公众号可用 `==重点==` 添加主题色重点下划线，小红书输出会自动移除标记符号
...
![公众号排版工作台](./docs/wechat-editorial-desktop.png)
...
小红书不支持颜色和字号，主题靠标题前缀、列表符号、强调括注和分隔线建立层级。下表是同一段 Markdown（小标题“小节”、加粗“关键”）在各主题下的实际输出：

| 主题 | 适合 | 小标题 | 强调 | 无序 / 有序 | 引用 | 分隔线 |
| --- | --- | --- | --- | --- | --- | --- |
| 清简 | 知识、观点 | `01｜小节` | `「关键」` | `•` / `①` | `摘录｜` | `······` |
| 信号 | AI、数码教程 | `▌ 小节` | `【关键】` | `→` / `❶` | `⌁ 提示｜` | `─── NOTE ───` |
| 纸间 | 生活方式 | `✦ 小节` | `﹝关键﹞` | `·` / `①` | `纸间记｜` | `· ✧ · ✧ ·` |
| 步序 | 清单、操作教程 | `步骤01｜小节` | `重点「关键」` | `✓` / `1️⃣` | `注意｜` | `— 操作分隔 —` |
| 元气 | 好物种草、探店 | `📍 小节` | `✨关键✨` | `✅` / `1️⃣` | `💬 ` | `✨ ✨ ✨` |
| 简报 | 职场干货、复盘 | `PART01 · 小节` | `『关键』` | `▸` / `1.` | `▍结论｜` | `━━━━━━━━` |
| 札记 | 读书、随笔 | `§ 小节` | `〖关键〗` | `◦` / `1.` | `书摘｜` | `❀ ❀ ❀` |
| 问答 | 科普、经验答疑 | `Q01. 小节` | `【关键】` | `◆` / `❶` | `A｜` | `· · · · · ·` |

文首标题作为笔记标题，不占用小标题编号，正文小标题始终从 01 开始；符号类主题会保留标题符号。

### 公众号：8 套版式气质 + 7 种标题样式

样式工坊先选一套完整气质，再单独调整任一参数。所有样式都以内联 CSS 输出，粘贴到公众号编辑器后基本保持原样，发布前仍建议在手机预览中检查。

| 版式气质 | 正文字体 | 字号 / 行距 | 主题色 | 标题样式 | 代码块 |
| --- | --- | --- | --- | --- | --- |
| 经典手记 | 无衬线 | 16 / 1.90 | 朱砂红 `#b44735` | 短线 | 深色墨块 |
| 东方书页 | 无衬线 | 16 / 1.90 | 陶朱红 `#a84535` | 通栏 | 浅色纸面 |
| 雅致长文 | 衬线 | 17 / 2.05 | 玫瑰金 `#a35f6f` | 通栏 | 浅色纸面 |
| 清爽简报 | 无衬线 | 15 / 1.75 | 墨绿 `#2f6f68` | 侧标 | 浅色纸面 |
| 技术蓝调 | 无衬线 | 15 / 1.75 | 经典蓝 `#185687` | 章签 | 深色墨块 |
| 黑白杂志 | 衬线 | 16 / 1.90 | 石墨黑 `#363636` | 序号 | 浅色纸面 |
| 森林笔记 | 无衬线 | 16 / 1.90 | 橄榄绿 `#5d7039` | 卡片 | 浅色纸面 |
| 薰衣草信笺 | 衬线 | 17 / 2.05 | 薰衣紫 `#8d5c7d` | 留白 | 浅色纸面 |

标题样式：短线、章签、侧标、通栏、留白、卡片、序号。预置主题色在白底上的对比度都不低于 4.5:1，也可以用取色器自定义。

![公众号样式工坊](./docs/wechat-style-workshop.png)
...
```bash
git clone https://github.com/smilexiaobao1992/zhijian-copy-studio.git
...
| mdast 节点 | 小红书输出规则 |
| --- | --- |
| `heading` | 根据主题生成标题前缀，再渲染行内内容；文首标题不占小标题编号 |
...
| 本地存储 | `localStorage`：多笔记 V2、旧稿迁移、420ms 防抖与 JSON 备份；IndexedDB：本地图片 |
...
```text
docs/                     README 截图等仓库资料
src/core/                 文档解析、双渠道主题协议、内容模板、渲染器与测试
src/features/editor/      React 编辑工作台、本地笔记与图片存储
...
- 公众号段落组件（提示框、对比卡片、作者签名等）
- 本地图片随 JSON 备份导出与恢复，删除笔记时清理无用图片
- 更多内容模板
- 主题配置导入、导出与社区共享
